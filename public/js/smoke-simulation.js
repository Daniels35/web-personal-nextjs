...
var config = {
  ...
  DENSITY_DISSIPATION: 0.995,  // Aumentado de 0.98 a 0.995
  VELOCITY_DISSIPATION: 0.995,  // Aumentado de 0.99 a 0.997 (opcional)
...
function getWebGLContext( canvas ) {
    ...
    gl.clearColor( 0.082, 0.082, 0.082, 1.0 );
...
function update() {
    ...
    gl.uniform2f( advectionProgram.uniforms.texelSize, 2.0 / textureWidth, 2.0 / textureHeight );
...
// Mantenemos el código para el contador y la generación de colores aleatorios
var count = 0;
var colorArr = [1, 1, 1]; // Establecemos el color como blanco

// Modificamos el movimiento para que siga suavemente una posición aleatoria
setInterval(function () {
    count++;
    (count > 25) && (colorArr = [1, 1, 1], count = 0); // Si deseas que el color siga siendo blanco, no necesitas cambiarlo

    pointers[0].down = true;
    pointers[0].color = colorArr;
    pointers[0].moved = pointers[0].down;

    // Generamos una posición aleatoria dentro del área del canvas
    var canvasWidth = canvas.width;
    var canvasHeight = canvas.height;
    var randomX = Math.random() * canvasWidth;
    var randomY = Math.random() * canvasHeight;

    // Calculamos las diferencias entre la posición aleatoria y la posición actual del objeto
    var diffX = randomX - pointers[0].x;
    var diffY = randomY - pointers[0].y;

    // Generamos las velocidades en función de las diferencias y la suavidad deseada
    pointers[0].dx = diffX * 1.10; // Puedes ajustar el factor de suavidad según sea necesario
    pointers[0].dy = diffY * 1.10; // Puedes ajustar el factor de suavidad según sea necesario

    // Actualizamos las coordenadas del puntero
    pointers[0].x += pointers[0].dx;
    pointers[0].y += pointers[0].dy;

    // Aseguramos que el objeto permanezca dentro del área del canvas
    pointers[0].x = (pointers[0].x + canvasWidth) % canvasWidth;
    pointers[0].y = (pointers[0].y + canvasHeight) % canvasHeight;
}, 200000 / 60); // Controla la velocidad de actualización del movimiento
